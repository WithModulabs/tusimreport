...
def generate_executive_summary(
    financial_data: dict, sentiment_data: dict, stock_code: str
) -> Dict[str, Any]:
    """재무·감정 데이터 기반 경영진 요약 보고서 생성"""
    try:
...
def generate_detailed_analysis_report(
    stock_code: str, company_name: str, financial_data: dict, sentiment_data: dict
) -> Dict[str, Any]:
    """LLM 기반 상세 투자 분석 보고서 생성"""
    try:
        logger.info(f"Generating detailed report for {stock_code}")

        # LLM으로 상세 보고서 생성 (Gemini or OpenAI)
        ...
        if provider == "gemini":
            ...
            report_llm = ChatOpenAI(model=model_name, temperature=0.3, api_key=api_key)

        # 입력 데이터 요약
        financial_summary = (
            str(financial_data)[:1000] if financial_data else "재무 데이터 없음"
        )
        sentiment_summary = (
            str(sentiment_data)[:1000] if sentiment_data else "감정 데이터 없음"
        )
        ...
        response = report_llm.invoke([HumanMessage(content=detailed_prompt)])
        ...
        if isinstance(sentiment_data, dict):
            if "error" in sentiment_data:
                ...
                if "부정" in sentiment_content:
                    ...
                    risk_level = "높음"
                if "뉴스" in sentiment_content and (
                    "적" in sentiment_content or "낮" in sentiment_content
                ):
                    risk_factors.append("뉴스 노출 부족")

        # 일반적 시장 리스크
        risk_factors.extend(
            ["한국 주식시장 변동성", "거시경제 환경 변화", "업종별 특수 리스크"]
        )
...
# LLM setup (Gemini or OpenAI)
provider, model_name, api_key = get_llm_model()

if provider == "gemini":
    llm = ChatGoogleGenerativeAI(
        model=model_name, temperature=0.4, google_api_key=api_key
    )
else:
    llm = ChatOpenAI(model=model_name, temperature=0.4, api_key=api_key)

# 한국 투자 보고서 ReAct Agent 생성
korean_report_react_agent = create_react_agent(
    ...
    prompt=(
        ...
        "CAPABILITIES:\n"
        "- Executive summaries for investment decisions\n"
        "- Detailed analysis reports in Korean\n"
        "- Risk assessments for Korean stocks\n"
        "- Comprehensive investment insights with disclaimers\n\n"
        "WORKFLOW:\n"
        "1. generate_executive_summary - Investment summary and recommendation\n"
        "2. generate_detailed_analysis_report - Comprehensive analysis\n"
        "3. generate_risk_assessment - Risk evaluation and mitigation\n"
        "4. Compile final investment report with disclaimers\n\n"
...
def generate_korean_stock_report(
    ...
    try:
        messages = [
            HumanMessage(
                content=f"Generate comprehensive investment report for Korean stock {stock_code} "
